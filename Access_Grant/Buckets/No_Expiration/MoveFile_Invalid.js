...
class MoveFileInvalidTests extends BaseTest {
    constructor() {
        super({
            testType: 'MOVE',
            timeout: 30000,
    ...
    async setup() {
        await super.setup();
        console.log('=================================');
        console.log('    Move File Invalid Tests     ');
        console.log('=================================');
    ...
    async runTest(testCase) {
        ...
        try {
            ...
            if (movedFile && testCase.config.permissions.delete) {
                try {
                    const storage = new StorageUtils(testCase.config, testCase.type);
    ...
    async run() {
        console.log('\n🚀 Starting Move File Invalid Tests\n');
...
if (require.main === module) {
    (async () => {
        const test = new MoveFileInvalidTests();
        await test.setup();
        await test.run();
        await test.teardown();
    })();
} else {
    module.exports = MoveFileInvalidTests;
} 